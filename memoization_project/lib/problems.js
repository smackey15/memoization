...
function lucasNumberMemo(n, memo = {}) {
    ...
    if (n in memo) return memo[n]; //fetching value of n if it's in the memo already
    if (n === 0) return 2;
    if (n === 1) return 1;

    memo[n] = lucasNumberMemo(n-1, memo) + lucasNumberMemo(n-2, memo); // storing value of n in the memo for future reference
    return memo[n];
}
